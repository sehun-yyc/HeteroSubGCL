...
# print("edges1!!",edges1)
# print("edges1.data.shape[0]!!",edges1.data.shape[0])
file_write_obj = open("./data/edges_gene_gbm.csv", 'w+')
for id in np.arange(edges1.data.shape[0]):
    file_write_obj.writelines(str(edges1.row[id]))
    file_write_obj.write(',')
    file_write_obj.writelines(str(edges1.col[id]))
    file_write_obj.write('\n')
    # print(str(edges1.row[id])+','+str(edges1.col[id]))
file_write_obj.close()

edges2=sp.coo_matrix(A2)
# print("edges2!!",edges2)
# print("edges2.data.shape[0]!!",edges2.data.shape[0])
file_write_obj = open("./data/edges_methy_gbm.csv", 'w+')
for id in np.arange(edges2.data.shape[0]):
    file_write_obj.writelines(str(edges2.row[id]))
    file_write_obj.write(',')
    file_write_obj.writelines(str(edges2.col[id]))
    file_write_obj.write('\n')
file_write_obj.close()

edges3=sp.coo_matrix(A3)
# print("edges3!!",edges3)
# print("edges3.data.shape[0]!!",edges3.data.shape[0])
file_write_obj = open("./data/edges_mirna_gbm.csv", 'w+')
for id in np.arange(edges3.data.shape[0]):
    file_write_obj.writelines(str(edges3.row[id]))
    file_write_obj.write(',')
    file_write_obj.writelines(str(edges3.col[id]))
    file_write_obj.write('\n')
file_write_obj.close()